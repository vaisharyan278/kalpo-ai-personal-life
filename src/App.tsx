...
import { getKalpanaResponse, getKalpanaAudio, resetKalpanaSession } from "./services/geminiService";
...
export default function App() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>(() => {
    const saved = localStorage.getItem("kalpana_chat_history");
    if (saved) {
      try {
  ...
  useEffect(() => {
    ...
    localStorage.setItem("kalpana_chat_history", JSON.stringify(messages));
  }, [messages]);
  ...
  const resetInactivityTimer = useCallback(() => {
    ...
    if (isSessionActive && appState === "listening") {
      inactivityTimeoutRef.current = setTimeout(async () => {
        if (isSessionActive && appState === "listening") {
          const nudge = "kya hua chup kiu ho";
          setMessages((prev) => [...prev, { id: Date.now().toString() + "-k", sender: "kalpana", text: nudge }]);
          
          if (liveSessionRef.current) {
            liveSessionRef.current.sendText("Nudge the user: 'kya hua chup kiu ho'");
          }
        }
      }, 60000); // 1 minute
    }
  ...
  const handleTextCommand = useCallback(async (finalTranscript: string) => {
    ...
    if (commandResult.isBrowserAction) {
      ...
      setMessages((prev) => [...prev, { id: Date.now().toString() + "-k", sender: "kalpana", text: responseText }]);
      
      if (!isMuted) {
        setAppState("speaking");
        const audioBase64 = await getKalpanaAudio(responseText);
        if (audioBase64) {
      ...
      responseText = await getKalpanaResponse(finalTranscript, messagesRef.current);
      setMessages((prev) => [...prev, { id: Date.now().toString() + "-k", sender: "kalpana", text: responseText }]);
  ...
  const toggleListening = async () => {
    if (isSessionActive) {
      ...
      try {
        ...
        session.onReconnectAttempt = () => {
          console.log("App: Session is attempting to reconnect...");
        };
    ...
    <div className="h-[100dvh] w-screen bg-[#050505] text-white flex flex-col items-center justify-between font-sans relative overflow-hidden m-0 p-0">
      ...
      <header className="absolute top-0 left-0 w-full flex justify-between items-center z-20 shrink-0 px-6 py-4 md:px-12 md:py-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-serif font-medium tracking-wide opacity-90">Kalpo</h1>
        </div>
          ...
          <AnimatePresence>
            {appState === "reconnecting" && (
              <motion.div
                ...
                className="px-3 py-1 bg-yellow-500/20 text-yellow-400 border border-yellow-500/30 rounded-full text-xs font-medium flex items-center gap-2"
              >
          ...
          {showTextInput && (
            ...
            >
              <input 
                ...
                placeholder="Type a message to Kalpo..."
